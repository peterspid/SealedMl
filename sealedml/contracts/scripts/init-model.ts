...
async function main() {
  ...
  const ModelRegistry = await ethers.getContractAt("ModelRegistry", MODEL_REGISTRY, wallet);
  ...
  const SealedMLInference = await ethers.getContractAt("SealedMLInference", SEALED_ML_INFERENCE, wallet);
  ...
  console.log(`Weights: ${modelInfo[0]}`);
